...
def show_info(message, title=None, notebook_off=False):
    title = f"⚠️ {title}" or "⚠️ This code is running in a Notebook!"
    if IS_NOTEBOOK and not notebook_off:
        html = f"""<div style="background:#ffe6e6;border:2px solid #d32f2f;padding:16px;border-radius:8px;text-align:center;">
            <span style="color:#d32f2f;font-size:1.5em;font-weight:bold;">{title}</span><br>
            <span style="color:#333;font-size:1.1em;">{message}</span>
        </div>"""
        display(HTML(html))
    else:
        underline = '=' * len(title)
...
class MockCMIP6Dataset:
    def __init__(self, transformers=None, warnings=None):
        ...
        self.spatial_extent = {"bbox": [-20, 10, 30, 50]}
        self.temporal_extent = {"interval": [["2020-01-01", "2050-12-31"]]}
...
class GeoCroissant:
    def __init__(self, provider="DEFAULT"):
        print(f"Initializing GeoCroissant client using provider: {provider}")
        self.version = "1.2.3"
    
    def search(self, keywords=None, spatial_coverage=None, temporal_range=None, facets=None):
...
croissant.GeoCroissant = GeoCroissant
